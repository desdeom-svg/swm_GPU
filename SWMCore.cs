...
namespace SWM
{
    public class SWMCore
    {
                ...
                for (int index2 = 0; index2 < slice.Scans.Count; ++index2)
                {
                   ...
                   scan.EnjaVir = wafer.FovMap[indexFov].FovPathType == AutoReviewSystem.Data.PathType.OnlyXYEqualPath;
                    ...
                    if (!sliceExtArea.IsEmpty)
                    {
                        ...
                        if (!area.IsEmpty)
                        {
                            AddExcludeIfItKeepsADieOwner(scan, area);
                        }
                    }
                    Rect scanExtArea;
                    //同一行每张图的行与上一张的交集
        ...
        /// <summary>
        /// 重叠区域只能从已有检测归属的 Die 中扣除。
        /// 如果当前 EXCLUDE 会把某个 Die 的完整 INCLUDE 遮掉，且此前没有可用的
        /// INCLUDE，则不添加该 EXCLUDE；这样同一 Die 至少保留一张图可检测。
        /// </summary>
        private void AddExcludeIfItKeepsADieOwner(Scan scan, Rect excludeArea)
        {
            List<IPROI> currentRois;
            if (!this.IPROIDictionary.TryGetValue(scan, out currentRois))
                return;

            foreach (IPROI include in currentRois.Where(roi => roi.IPROIType == Base.ID_IPROIType.INCLUDE))
            {
                if (excludeArea.Contains(include.Area) && !HasPreviousEffectiveInclude(scan, include.DieID))
                    return;
            }

            currentRois.Add(new IPROI(scan, excludeArea, Base.ID_IPROIType.EXCLUDE, 99999999));
        }

        private bool HasPreviousEffectiveInclude(Scan currentScan, int dieId)
        {
            foreach (KeyValuePair<Scan, List<IPROI>> item in this.IPROIDictionary)
            {
                if (item.Key.Index >= currentScan.Index)
                    continue;

                foreach (IPROI include in item.Value.Where(roi => roi.IPROIType == Base.ID_IPROIType.INCLUDE && roi.DieID == dieId))
                {
                    bool completelyExcluded = item.Value.Any(roi =>
                        roi.IPROIType == Base.ID_IPROIType.EXCLUDE && roi.Area.Contains(include.Area));
                    if (!completelyExcluded)
                        return true;
                }
            }
